...
def make_folder(target_root, product, color):
    try:
        if not os.path.exists(os.path.join(target_root, 'images', 'segmentation', product, color)):
            ...
            os.makedirs(os.path.join(target_root, 'clothes', 'base', product, color))
            os.makedirs(os.path.join(target_root, 'clothes', 'mask', product, color))
...
for i, product in enumerate(products):
    ...
    for j, color in enumerate(colors):
        ...
        for k, item in enumerate(items):
            ...
            plt.imshow((img * 255).astype(numpy.uint8))
            plt.show()
            ...
            if opinion == 'x':
                ...
                shutil.move(os.path.join(data_origin_root, 'clothes', 'mask', product, color, item), os.path.join(target_root, 'clothes', 'mask', product, color, item[:-4] + '_mask.png'))
                ...
                shutil.move(os.path.join(data_origin_root, 'images', 'mask', product, color, item), os.path.join(target_root, 'images', 'mask', product, color, item[:-4] + '_mask.png'))
            else:
                print('제대로 입력해줘잉')
